...
for the DrishtiIR Diffusion model on the validation set.

Usage:
    python evaluate_final.py --ir-dir data/train_massive/ir_multiband \
                             --rgb-dir data/train_massive/rgb \
                             --ckpt checkpoints/diffusion_ema_latest.pth \
                             --n-samples 50
...
def load_diffusion(ckpt_path: str, device: str):
    ...
    print(f"  [EVAL] Diffusion model loaded from {ckpt_path}")
    return model


@torch.no_grad()
def run_ddim(model, ir_t, scheduler, guidance_scale, device):
...
def main():
    ...
    parser.add_argument("--ckpt",        default="checkpoints/diffusion_ema_latest.pth")
    parser.add_argument("--n-samples",   type=int, default=50)
    parser.add_argument("--ddim-steps",  type=int, default=20)
    parser.add_argument("--guidance",    type=float, default=2.0)
    parser.add_argument("--out",         default="results/evaluation_report.json")
    args = parser.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"  [EVAL] Device: {device}")

    model     = load_diffusion(args.ckpt, device)
    scheduler = get_ddim_scheduler(num_inference_steps=args.ddim_steps)

    loader, _ = get_dataloader(
        ir_dir=args.ir_dir, rgb_dir=args.rgb_dir,
        batch_size=1, num_workers=2,
        image_size=256, tiled=False, augment=False,
    ...
    for ir_t, rgb_t in tqdm(loader, total=min(args.n_samples, len(loader)), desc="Evaluating"):
        ...
        fake = run_ddim(model, ir_t, scheduler, args.guidance, device)
    ...
    report = {
        "model":           "DrishtiIR Diffusion EMA (DDPM + DDIM + CFG)",
    ...
    print("\n" + "="*60)
    print("  FINAL EVALUATION RESULTS — DrishtiIR Diffusion Model")
    print("="*60)
    m = report["metrics"]
